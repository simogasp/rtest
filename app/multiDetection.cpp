...
bool parseArgs( int argc, char**argv, string &inputFilename, int &cameraID, bool &useCamera, string &patternFilename, string &calibFile );  
...
int main( int argc, char** argv )
{
	...
	// it will contains the filename of multipattern configuration
    ...
    MultiARTagBasedTracker tracker;

    if( !parseArgs( argc, argv, inputFilename, cameraID, useCamera, patternFilename, calibFilename) )
    {
    	cerr << "Aborting..." << endl;
    	return EXIT_FAILURE;
    }
	...
	cam.init( patternFilename );
	
	MultiARTagBasedTracker::Parameters param;
	param._multiPatternFilename = patternFilename;
	tracker.init( param, cam );
...
void help( const char* programName )
{
    cout << "Detect the artags" << endl
		...
		<< "     -p <multi pattern file>                           # the file containing the multi pattern .cfg" << endl
		<< "     <video file|camera index>                         # the name of the video file or the camera id number for a live feed" << endl
        << endl;
}

// parse the input command line arguments
bool parseArgs( int argc, char**argv, string &inputFilename, int &cameraID, bool &useCamera, string &patternFile, string &calibFile )
{
